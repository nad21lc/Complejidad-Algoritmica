...
def calcular_componente_mayor(file):
    #"2 4"
    # rsplit lo va dividir por el espacio vacio en el arreglo 
    # ["2", "4"]
    # map recorre el arreglo le asigna los valores a n y m respectivamente
    n, m = map(int, file.readline().rstrip().split())
    #quiero leer de 1 hasta n (donde empiezan los amigos)
    # el _ quiere decir que no lo voy a usar
...
with open("in1.txt", "r") as file:
    #readline() te da el num como cadena de texto y da el salto de linea
    #pero con rstrip() ya no salto
    #con int lo convierto de string a text
